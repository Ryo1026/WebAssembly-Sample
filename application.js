...
application = function(options) {
    ...
    // Constant
    this.URL_SETTIME = "http://{HOSTNAME}:{HTTPPORT}/Media/SyncPlayback/setcurrenttime?syncsession={SESSION}&currenttime={TIME}";
    this.URL_SETMODE = "http://{HOSTNAME}:{HTTPPORT}/Media/SyncPlayback/SetPlayMode?syncsession={SESSION}&action={ACTION}&playrate={RATE}";
    this.URL_PLAY = "http://{HOSTNAME}:{HTTPPORT}/Media/SyncPlayback/Play?syncsession={SESSION}";
    this.URL_PAUSE = "http://{HOSTNAME}:{HTTPPORT}/Media/SyncPlayback/Pause?syncsession={SESSION}";
    this.URL_STEPPREVIOUS = "http://{HOSTNAME}:{HTTPPORT}/Media/SyncPlayback/StepPrevious?syncsession={SESSION}";
    this.URL_STEPNEXT = "http://{HOSTNAME}:{HTTPPORT}/Media/SyncPlayback/StepNext?syncsession={SESSION}";
...
aui.lang.Class.extend(application, aui.ui.ControlBase, {
    ...
    prepareNode: function(nodeToAppend) {
        ...
        function InitialUI() {
            if (!uiInitialized &&
                ...
                var code = me.pvtUtilityAdapterModule.dataTranscode("DES_V1_ENC", "Aa123456");
                console.log("encode = " + code);
                var str = me.pvtUtilityAdapterModule.dataTranscode("DES_V1_DEC", code);
                console.log("decode = " + str);

                code = '<?xml version="1.0" encoding="UTF - 8"?><Message type="Event" Time="1574653185.127" Bias=" - 480" DaylightBias="0"><Event type="Status" srctype="Camera" src="5" id="VideoStatus" port="1" active="on"/></Message>';
                console.log("encode = " + code);
        ...
        if (typeof WebAssembly === "undefined" || typeof fetch === "undefined") {
            ...
            //Fragment
            fetch(me.PATH_WEBASSEMBLY_FRAGMENT_MODULE, { cache: 'no-cache' }).then(function(response) {
			    if (!response["ok"]) {
            ...
            //Fisheye
            fetch(me.PATH_WEBASSEMBLY_FISHEYE_MODULE, { cache: 'no-cache' }).then(function (response) {
                if (!response["ok"]) {
            ...
            //WebGLAdapter
            fetch(me.PATH_WEBASSEMBLY_WEBGLADAPTER_MODULE, { cache: 'no-cache' }).then(function (response) {
                if (!response["ok"]) {
                    alert("Fetch " + me.PATH_WEBASSEMBLY_WEBGLADAPTER_MODULE + " Failed");
                }
                else {
                    return response.arrayBuffer();
                }
            ...
            fetch(me.PATH_WEBASSEMBLY_UTILITYADAPTER_MODULE, { cache: 'no-cache' }).then(function (response) {
                if (!response["ok"]) {
                    alert("Fetch " + me.PATH_WEBASSEMBLY_UTILITYADAPTER_MODULE + " Failed");
                }
                else {
                    return response.arrayBuffer();
                }
    ...
    initWebAssemblyController: function () {
        ...
        for (var i = 0; i < me.pvtControlCount; i++) {
            var eventList = [
                ...
                'onmousedown', 'onmousemove', 'onmouseover', 'onmouseup', 'onmouseout',
                'onselect', 'onsubmit', 'oncontextmenu'
            ];
            let mdlid = (i + 1);
            var fragObject = me.pvtStreamFragment.createFragmentObject(mdlid);
            fragObject.createWorker(me.pvtStreamFragment.URL_FRAGWORKER + "?v=" + (new Date()).getTime());    // set FragWorker.js path
            me.pvtControllers[i] = new aui.nvr.ui.WebAssemblyController({ object: fragObject, sessionKey: me.pvtSessionKey, events: eventList, id: mdlid });
    ...
    initControlPanel: function () {
        ...
        me.pvtControlPanel.onFisheyeMountingTypeChanged.subscribe(function (type, args) {
    ...
    changeFisheyeModuleAll: function (value) {
        ...
        {
            for (var i = 0; i < me.pvtControlCount; i++) {
                me.pvtControllers[i].setFisheyeObject(me.pvtFisheyeModule.createFishEyeObject(value));
                var fisheyeCircle = me.pvtFisheyeModule.getFishEyeDefaultCircle(value);
                me.pvtControllers[i].setFisheyeCircle(fisheyeCircle.x, fisheyeCircle.y, fisheyeCircle.r, fisheyeCircle.width, fisheyeCircle.height);
            }
        }
    },
    ...
    setCurrentTime: function (data) {
        ...
        var url = me.URL_SETTIME.replace("{HOSTNAME}", serverIP).replace("{HTTPPORT}", serverPort).replace("{TIME}", playbackTime).replace("{SESSION}", me.pvtSessionKey);
        me._sendURLCommand(url, account, password);
    },
    ...
    rewind: function() {
        ...
        var url = "";
        if (rate > 0) {
            url = me.URL_PLAY.replace("{HOSTNAME}", serverIP).replace("{HTTPPORT}", serverPort).replace("{SESSION}", me.pvtSessionKey);
        } else {
            // frame by frame, step previous
            url = me.URL_STEPPREVIOUS.replace("{HOSTNAME}", serverIP).replace("{HTTPPORT}", serverPort).replace("{SESSION}", me.pvtSessionKey);
        }
        me._sendURLCommand(url, account, password);
    },
    ...
    play: function() {
        ...
        if (rate > 0) {
            ...
            url = me.URL_STEPNEXT.replace("{HOSTNAME}", serverIP).replace("{HTTPPORT}", serverPort).replace("{SESSION}", me.pvtSessionKey);
        }
        me._sendURLCommand(url, account, password);
    },
    ...
    pause: function() {
        ...
        var url = me.URL_PAUSE.replace("{HOSTNAME}", serverIP).replace("{HTTPPORT}", serverPort).replace("{SESSION}", me.pvtSessionKey);
        me._sendURLCommand(url, account, password);
    },
    ...
    setPlayRate: function(rate) {
        ...
        var url = me.URL_SETMODE.replace("{HOSTNAME}", serverIP).replace("{HTTPPORT}", serverPort).replace("{SESSION}", me.pvtSessionKey).replace("{ACTION}", action).replace("{RATE}", rate);
        me._sendURLCommand(url, account, password);
    },
        ...
        if (httpRequest) {
            try {
                ...
                if (account) {
                    ...
                    httpRequest.setRequestHeader("Authorization", "Basic " + Base64.encode(account + ":" + password));
                }
    ...
    function onMouseWheel(ev) {
        for (var i = 0; i < app.pvtWorkingControlCount; i++) {
            if (app.pvtControllers[i]) {
                ...
                if (ev.clientX >= controlRegion.left && ev.clientX < controlRegion.right && ev.clientY >= controlRegion.top && ev.clientY < controlRegion.bottom) {
